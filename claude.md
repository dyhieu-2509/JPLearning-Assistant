...
16. **Frontend portal split** — Learner app và Admin console phải tách route/layout/source tree rõ ràng: learner ở `/learner`, admin ở `/admin`; learner-owned code nằm dưới `features/learner` và `pages/learner`, admin-owned code nằm dưới `features/admin` và `pages/admin`, auth nằm riêng dưới `features/auth` và `pages/auth`. Không trộn công cụ quản trị vào trang người học; admin route phải có role guard trước khi gọi API quản trị.
17. **Learner onboarding first** — Learner mới phải đi qua `/learner/onboarding` trước dashboard nếu profile vẫn ở default. Onboarding dùng khoảng 5-10 câu hỏi profile setup, hiện là 8 câu: current level, target level, goal, deadline, daily study minutes, weak skills, explanation style, romaji preference. Kết quả lưu qua `/api/v1/personalization/me/profile` để chatbot, planner, flashcard và assessment dùng cùng nguồn cá nhân hóa.

...
- Learner profile includes `learningPathway` (`jlpt_foundation`, `conversation`, `school`, `work`, `reading`). Planner and tutor context must use it to choose pathway-specific tasks while still combining weak progress, due flashcards, recent chat topics, and latest assessment.
...
16. **Frontend portal split** — Learner app và Admin console phải tách route/layout/source tree rõ ràng: public ở `/` và `/onboarding`, learner ở `/learner`, admin ở `/admin`; public-owned code nằm dưới `features/public` và `pages/public`, learner-owned code nằm dưới `features/learner` và `pages/learner`, admin-owned code nằm dưới `features/admin` và `pages/admin`, auth nằm riêng dưới `features/auth` và `pages/auth`. Không trộn công cụ quản trị vào trang người học; admin route phải có role guard trước khi gọi API quản trị.
17. **Learner onboarding first, assessment for recalibration** — Flow mặc định là landing page `/` → `Get Started` → 2 lựa chọn: `New Learner` làm 8 câu onboarding trước đăng nhập tại `/onboarding`, `Current Learner` đi thẳng `/login?mode=login`. Pre-auth onboarding lưu draft local và sync qua `/api/v1/personalization/me/profile` ngay sau khi login/register/OAuth có JWT. Authenticated `/learner/onboarding` chỉ dùng khi learner chưa có đủ profile ban đầu; không expose menu chỉnh lại onboarding thủ công cho người đã được cá nhân hóa. Nếu cần cá nhân hóa lại, learner phải làm assessment/quiz để tạo tín hiệu đáng tin cậy, sau đó hệ thống dùng kết quả kiểm tra, progress, flashcard và chat history để điều chỉnh cá nhân hóa. Onboarding dùng khoảng 5-10 câu hỏi profile setup, hiện là 8 câu: current level, target level, learning pathway, deadline, daily study minutes, weak skills, explanation style, romaji preference.

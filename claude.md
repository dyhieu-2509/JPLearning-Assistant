...
17. **Learner onboarding first** — Flow mặc định là landing page `/` → `Get Started` → 2 lựa chọn: `New Learner` làm 8 câu onboarding trước đăng nhập tại `/onboarding`, `Current Learner` đi thẳng `/login?mode=login`. Pre-auth onboarding lưu draft local và sync qua `/api/v1/personalization/me/profile` ngay sau khi login/register/OAuth có JWT; learner vẫn có thể chạy lại `/learner/onboarding`. Onboarding dùng khoảng 5-10 câu hỏi profile setup, hiện là 8 câu: current level, target level, goal, deadline, daily study minutes, weak skills, explanation style, romaji preference.

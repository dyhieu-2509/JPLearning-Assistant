  ...
  async function finish() {
    ...
    try {
      await apiRequest("/personalization/me/profile", {
        method: "PUT",
        token: accessToken,
        body: toProfileRequest(answers)
      });
      navigate("/learner", { replace: true });
    } catch (caught) {
      setError(caught instanceof ApiError ? caught.message : "Cannot save onboarding profile");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <main className="onboarding-shell">
      <section className="onboarding-hero">
        ...
        <h2>Thiết lập cá nhân hóa ban đầu</h2>
        <p>
          VAJA cần vài tín hiệu học tập để chọn độ khó, cách giải thích và lộ trình ôn tập phù hợp ngay từ buổi đầu.
        </p>
...
function toProfileRequest(answers: OnboardingAnswers): StudentProfileRequest {
  return {
